...
export class PricingEngine {
  ...
  constructor(currencies: Array<{code, rateToBase}>, resources: ResourceRate[]) {
    this.currencyRates = new Map(currencies.map(c => [c.code, c.rateToBase]));
    this.resourceRates = new Map(resources.map(r => [r.id, r]));
  }

  calculateBaseCost(packages: SAPPackage[], targetCurrency = "SGD") {
  ...
  private getBlendedDayRate(category, targetCurrency) {
