// src/adapters/ai/cloud/HuggingFaceProvider  
export class HuggingFaceProvider implements AIProvider {
  name = 'HuggingFace Inference';
  type = 'cloud-huggingface' as const;
  
  private apiKey | null = null;
  ...
  private buildStructuredPrompt(request: EstimationRequest) {
    return `
...
3. Key recommendations
  ...
  private parseHuggingFaceResponse(data, request: EstimationRequest): EstimationResult {
  ...
  private fallbackEffortEstimate(request: EstimationRequest) {
  ...
  private extractRisksFromText(text)[] {
    const risks = [];
