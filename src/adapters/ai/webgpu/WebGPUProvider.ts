// src/adapters/ai/webgpu/WebGPUProvider.ts
...
export class WebGPUProvider implements AIProvider {
  ...
  private requestToFeatureVector(request: EstimationRequest): number[] {
  ...
  private calculatePackageComplexity(packages: string[]): number[] {
  ...
  private async generateBreakdown(request: EstimationRequest, totalEffort: number): Promise<EstimationResult['breakdown']> {
    const breakdown = [];
  ...
  private getPackageMultiplier(packageId: string): number {
  ...
  private getPackageReasoning(packageId: string): string {
  ...
  private getPackageRisks(packageId: string, profile: Partial<ClientProfile>): string[] {
    const risks = [];
