...
const SAPScopeAppEnhanced = () => {
  ...
  const handleModuleToggle = (packageId, moduleId) => {
    const pkg = state.packages.find(p => p.id === packageId);
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: '16px', fontSize: '13px', color: '#8e8e93' }}>
                      {pkg.total_effort_pd}</strong> person-days</span>
                      {pkg.modules.length}</strong> modules</span>
                    </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
            ...
            <div style={{ fontSize: '14px', color: '#6e6e73' }}>
              {state.packages.filter(p => p.selected).length}</strong> packages • 
               {state.packages.flatMap(p => p.modules.filter(m => m.selected)).length}</strong> modules • 
               {state.malaysiaForms.filter(f => f.selected).length}</strong> forms • 
               {totalEffort.toFixed(1)}</strong> person-days • 
               {riskAssessment.overall_risk}</strong> risk
            </div>
          </div>
